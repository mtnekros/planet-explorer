...
bool handle_planet_discovery( player_data& player, std::vector<planet_data>& planets )
{
    ...
    {
        if ( sprite_collision( player.player_sprite, planet.planet_sprite ) && !planet.planet_visited )
        {
            planet.planet_visited = true;
            player.score++;
            return true;
        }
    }
...
void handle_asteroid_collision( player_data& player, std::vector<Asteroid>& asteroids )
{
    for ( Asteroid& asteroid : asteroids )
    {
        if ( sprite_collision( player.player_sprite, asteroid.a_sprite ) )
        {
            player.health -= 0.04f;
        }
    }
